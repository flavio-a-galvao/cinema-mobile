...
export default function AdminScreen() {
  ...
  return <Screen>
    ...
    <Button icon="add-outline" title="Novo filme" disabled={pending !== null} onPress={() => router.push(adminMovieRoute())} />
    ...
    {movies?.map(movie => <View key={movie.id_filme} style={styles.card}>
      ...
      <View style={styles.actions}><View style={styles.action}><Button icon="open-outline" title="Visualizar" variant="link" onPress={() => router.push(movieRoute(movie.id_filme))} /></View>
      <View style={styles.action}><Button icon="create-outline" title="Editar" variant="secondary" disabled={pending !== null} onPress={() => router.push(adminMovieRoute(movie.id_filme))} /></View></View>
      <Button title="Excluir" icon="trash-outline" variant="danger" loading={pending === movie.id_filme} disabled={pending !== null} onPress={() => confirmDelete(movie)} />
    </View>)}
...
const createStyles = (theme: AppTheme) => StyleSheet.create({
  ...
  card: { borderWidth: theme.sizes.borderWidth, borderColor: theme.colors.border, backgroundColor: theme.colors.surface, borderRadius: theme.radius.lg, padding: theme.spacing.md, gap: theme.spacing.sm },
  actions: { flexDirection: 'row', flexWrap: 'wrap', gap: theme.spacing.sm }, action: { flexGrow: 1, flexBasis: '45%' },
