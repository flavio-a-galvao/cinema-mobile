...
const serialize = (ticket: Ingresso) => ({ id_ingresso: Number(ticket.get('id_ingresso')), id_sessao: Number(ticket.get('id_sessao')), id_cliente: Number(ticket.get('id_cliente')), id_assento: Number(ticket.get('id_assento')), data_compra: ticket.get('data_compra'), status: ticket.get('status'), cancelado_em: ticket.get('cancelado_em') });
  ...
  try {
   ...
   const created = await sequelize.transaction(async transaction => {
    ...
    for (const seatId of seats) {
     ...
     tickets.push(await Ingresso.create({ id_sessao: sessionId, id_cliente: clientId, id_assento: seatId, status: 'ativo' }, { transaction }));
    }
    return tickets;
   });
